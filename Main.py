...
from Backend.SpeechToText import SpeachRecognition
...
def ChatLogIntegration():
    ...
    with open(os.path.join(TempDirPath, 'Database.json'), "w", encoding='utf-8') as file:
        file.write(AnswerModifier(formatted_chatlog))


def ShowChatOnGUI():
    with open(os.path.join(TempDirPath, 'Database.json'), "r", encoding='utf-8') as File:
        Data = File.read()
    if len(str(Data))>0:
        lines = Data.split('\n')
        result = json.dumps(lines)
        File.close()
        File = open(os.path.join(TempDirPath, 'Responses.data'), "w", encoding='utf-8')
        File.write(result)
        File.close()
...
def MainExecution():
    ...
    if ImageExcution == True:
        ...
        try:
            ...
            print(f"Error starting ImaageGeneration.py: {e}")
